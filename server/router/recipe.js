...
// 내 정보 (내 레시피) 정보 뽑아오기
router.get('/my/category/', saveController.getByType); // /?id=국&userid=apple로 창에 검색해야함
router.get('/my/detail/', saveController.getRecipe);   // /?id=국&userid=apple로 창에 검색해야함


router.post('/saveData',saveController.saveData);
router.delete('/deleteData/',saveController.deleteData);    // /?id=국&userid=apple로 창에 검색해야함
